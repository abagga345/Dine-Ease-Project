...
export function Addresses() {
  ...
  useEffect(() => {
    ...
    async function fetchaddress() {
      try {
        let result = await axios.get("http://localhost:3000/api/v1/user/getaddresses", {
          headers: {
            Authorization: token,
          },
        });
  ...
  return (
    <div className="w-full h-full">
      <div className="font-semibold text-3xl w-full text-center my-4">Saved Addresses</div>
      ...
      <div className="bg-gray-50 px-10 pt-10 pb-20 mt-6 mb-20 w-[80%] mx-auto rounded-xl border border-gray-100">
        {loading ? (
          ...
          <div className="flex flex-col">
            ...
            <AddAddress setAddresses={setAddresses} navigate={navigate} setError={setError} />
          </div>
        )}
      </div>
    </div>
